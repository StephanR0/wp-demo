...
class Foodhunt_Theme_Review_Notice {
	...
	public function review_notice() {
		...
		if ( ! get_option( 'foodhunt_theme_installed_time' ) ) {
	...
	public function review_notice_markup() {
		...
		$dismiss_url              = wp_nonce_url(
			add_query_arg( 'nag_foodhunt_ignore_theme_review_notice', 0 ),
			'nag_foodhunt_ignore_theme_review_notice_nonce',
			'_foodhunt_ignore_theme_review_notice_nonce'
		);
		$temporary_dismiss_url    = wp_nonce_url(
			add_query_arg( 'nag_foodhunt_ignore_theme_review_notice_partially', 0 ),
			'nag_foodhunt_ignore_theme_review_notice_partially_nonce',
			'_foodhunt_ignore_theme_review_notice_nonce'
		);
		/**
		 ...
		 */
		if ( ( get_option( 'foodhunt_theme_installed_time' ) > strtotime( '-14 day' ) ) || ( $ignored_notice_partially > strtotime( '-14 day' ) ) || ( $ignored_notice ) ) {
			return;
		}
		?>
		<div class="notice notice-success foodhunt-notice theme-review-notice" style="position:relative;">
			<div class="foodhunt-message__content">
				<div class="foodhunt-message__image">
					<img class="foodhunt-logo--png" src="<?php echo esc_url( get_template_directory_uri() . '/inc/admin/images/foodhunt-square-logo.png' ); ?>" alt="<?php esc_attr_e( 'FoodHunt', 'foodhunt' ); ?>" />
				</div>
				...
				<div class="foodhunt-message__text">
					...
					<p>(
						...
						printf(
							...
							esc_html__( 'The above word is just to draw your attention. %s', 'foodhunt' ),
					...
					<p>
						<?php
							printf(
								...
								esc_html__( 'Hope you are having a nice experience with %1$s %2$s %3$s theme. Please provide this theme a nice review.', 'foodhunt' ),
								'<strong>',
					...
					<strong>
						<?php esc_html_e( 'What benefit would you have?', 'foodhunt' ); ?>
					</strong>
					<p>
						<?php
							printf(
								/* translators: %s: Smiley icon */
								esc_html__( 'Basically, it would encourage us to release updates regularly with new features & bug fixes so that you can keep on using the theme without any issues and also to provide free support like we have been doing. %s', 'foodhunt' ),
								'<span class="dashicons dashicons-smiley smile-icon"></span>'
							);
						?>
					</p>
					...
					<div class="links">
						<a href="https://wordpress.org/support/theme/foodhunt/reviews/?filter=5#new-post" class="btn button-primary" target="_blank">
							...
							<span><?php esc_html_e( 'Sure, I\'d love to!', 'foodhunt' ); ?></span>
						</a>
						...
						<a href="<?php echo esc_url( 'https://wordpress.org/support/theme/foodhunt/' ); ?>" class="btn button-secondary" target="_blank">
							<span class="dashicons dashicons-testimonial"></span>
							<span><?php esc_html_e( 'I have a query', 'foodhunt' ); ?></span>
						</a>
					</div> <!-- /.links -->
				</div> <!-- /.foodhunt-message__text -->

				<a class="notice-dismiss" href="<?php echo esc_url( $dismiss_url ); ?>"></a>

			</div> <!-- /.foodhunt-message__content -->
	...
	public function ignore_theme_review_notice() {
		...
		if ( isset( $_GET['nag_foodhunt_ignore_theme_review_notice'] ) && isset( $_GET['_foodhunt_ignore_theme_review_notice_nonce'] ) ) {

			if ( ! wp_verify_nonce( wp_unslash( $_GET['_foodhunt_ignore_theme_review_notice_nonce'] ), 'nag_foodhunt_ignore_theme_review_notice_nonce' ) ) {
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'foodhunt' ) );
			}

			if ( '0' === $_GET['nag_foodhunt_ignore_theme_review_notice'] ) {
				add_user_meta( get_current_user_id(), 'foodhunt_ignore_theme_review_notice', 'true', true );
			}
		}
	}
	...
	public function ignore_theme_review_notice_partially() {
		...
		/* If user clicks to ignore the notice, add that to their user meta */
		if ( isset( $_GET['nag_foodhunt_ignore_theme_review_notice_partially'] ) && isset( $_GET['_foodhunt_ignore_theme_review_notice_nonce'] ) ) {

			if ( ! wp_verify_nonce( wp_unslash( $_GET['_foodhunt_ignore_theme_review_notice_nonce'] ), 'nag_foodhunt_ignore_theme_review_notice_partially_nonce' ) ) {
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'foodhunt' ) );
			}

			if ( '0' === $_GET['nag_foodhunt_ignore_theme_review_notice_partially'] ) {
				update_user_meta( get_current_user_id(), 'nag_foodhunt_ignore_theme_review_notice_partially', time() );
			}
		}
	}
